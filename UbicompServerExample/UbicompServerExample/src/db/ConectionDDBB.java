...
public class ConectionDDBB
{
    ...
    public static void GetTiempo(Connection con, int idPlaca){
        PreparedStatement consulta = getStatement(con,"SELECT TOP 1 * FROM TIEMPO ORDER BY FECHA DESC WHERE IDPLACA = "+idPlaca+";");
        MQTTBroker broker = new MQTTBroker();
        MQTTPublisher.publish(broker, "BBDD", consulta.toString());
    }
    
    public static PreparedStatement GetUserInfo(Connection con){
        return getStatement(con, "select * from USER;");
    }
    
    public static PreparedStatement GetPlacas(Connection con){
        return getStatement(con, "select * from PLACA;");
    }
